...
export function evaluate(dependency, baseline, registryData, policy) {
  ...
  // Extract the specific lifecycle script names if available from registry metadata.
  const scriptNames = registryData?.scripts
    ? Object.keys(registryData.scripts).filter((k) => /^(pre|post)?install$/.test(k))
    : [];
