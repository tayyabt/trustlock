...
## Description
Implement `dep-fence init` — the cross-cutting onboarding command that creates the project configuration file, the `.dep-fence/` directory scaffold, and the initial trust baseline from the current lockfile. This story covers the init-onboarding workflow in full.

## Scope
**In scope:**
- `src/cli/commands/init.js` — full implementation (replace stub from F08-S1)
- Creating `.depfencerc.json` with default policy configuration
- Creating `.dep-fence/` directory structure: `approvals.json` (empty array), `.cache/`, `.gitignore`
...
- `--strict` flag: write a stricter default `.depfencerc.json` (provenance required for top packages)
- `--no-baseline` flag: skip baseline creation entirely (write directory scaffold only)
- Trusting all current packages by default (D6: error if `.dep-fence/` already exists)
- Final summary message and next-step guidance

**Not in scope:**
- `install-hook` — separate story (F08-S5)
- `--force` flag (nice-to-have per PM assumptions; deferred — user deletes `.dep-fence/` manually)
- Registry caching during init (cache is freshly created here, so cache-first only applies to subsequent runs)

## Entry Points
- Route / page / screen: `dep-fence init [--strict] [--no-baseline]`
...
- Boundary / contract check: After `dep-fence init` runs, `dep-fence check` must work without errors (verified in F08-S6 integration test)
...
- `.dep-fence/.gitignore` creation — must be real and must gitignore the `.cache/` directory (D8)

## Behavioral / Interaction Rules
- **D6 (init fails if .dep-fence/ exists):** Exit 2 with "dep-fence is already initialized. Delete `.dep-fence/` to reinitialize."
...
- **`--strict`:** write `.depfencerc.json` with `provenance.required: true` and higher default thresholds instead of permissive defaults
- **`--no-baseline`:** create directory scaffold and `.depfencerc.json` but do NOT parse lockfile or write `baseline.json`; print "Skipped baseline creation. Run `dep-fence audit` to review your dependency posture before running `dep-fence check`."
...
- Summary message: `"Baselined N packages. Detected npm lockfile vX. Next: run 'dep-fence install-hook' to enable the pre-commit hook."`

## Acceptance Criteria
- [ ] `dep-fence init` creates `.depfencerc.json` in project root with valid default policy
- [ ] `dep-fence init` creates `.dep-fence/` containing `approvals.json` (`[]`), `.cache/` directory, `.gitignore` (caching `.cache/`, D8)
- [ ] `dep-fence init` creates `.dep-fence/baseline.json` with all current packages trusted
- [ ] `dep-fence init` prints "Baselined N packages. Detected npm lockfile vX." with correct counts
- [ ] `dep-fence init` when `.dep-fence/` already exists exits 2 with "already initialized" message (D6)
- [ ] `dep-fence init` with no lockfile exits 2 with "No lockfile found" message
- [ ] `dep-fence init` with unknown lockfile version exits 2 (Q1 — fail hard)
- [ ] `dep-fence init --strict` creates `.depfencerc.json` with stricter policy thresholds
- [ ] `dep-fence init --no-baseline` creates scaffold and config but not `baseline.json`
- [ ] `dep-fence init` with registry unreachable creates baseline (with null provenance) and prints warning

## Task Breakdown
1. Implement `src/cli/commands/init.js` — guard: check `.dep-fence/` existence (D6), detect lockfile
2. Write `.depfencerc.json` with default or `--strict` policy config
3. Create `.dep-fence/` directory, `approvals.json`, `.cache/`, `.gitignore` (D8)
...
# Expected: "Baselined N packages. Detected npm lockfile v3. Next: run 'dep-fence install-hook' ..."

ls .dep-fence/
# Expected: approvals.json  .cache/  .gitignore  baseline.json

node -e "console.log(JSON.parse(require('fs').readFileSync('.depfencerc.json', 'utf8')))"
# Expected: valid policy config object

node src/cli/index.js init; echo $?
# Expected (second run): "dep-fence is already initialized.", exits 2
...
- `.dep-fence/` already exists: exit 2 (D6)
