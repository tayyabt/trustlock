...
test('loadPolicy: valid sparse config — missing fields filled from defaults', async () => {
  ...
  // Default allowlist is pre-populated with known packages that legitimately use install scripts
  assert.ok(Array.isArray(policy.scripts.allowlist), 'allowlist should be an array');
  assert.ok(policy.scripts.allowlist.includes('esbuild'), 'default allowlist should include esbuild');
  assert.ok(policy.scripts.allowlist.includes('sharp'), 'default allowlist should include sharp');
...
test('loadPolicy: unknown rule names in config are ignored — no error, known fields merged', async () => {
  ...
  try {
    ...
    // Defaults fill in the rest (allowlist is pre-populated with known packages)
    assert.ok(Array.isArray(policy.scripts.allowlist), 'allowlist should be an array');
    assert.ok(policy.scripts.allowlist.includes('esbuild'), 'default allowlist should include esbuild');
    assert.deepEqual(policy.sources.allowed, ['registry']);
