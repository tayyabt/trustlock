...
3. **Relaxed JSON uses `_comment` fields**: JSON does not support comments. The `relaxed.trustlockrc.json` uses `_comment` fields (ignored by `loadPolicy`'s `mergeNested` because they are not in the known-key set) to annotate each relaxed setting.
4. **CI examples use Node.js 18.3+ and `--enforce`**: All CI workflow examples specify `node-version: '18.x'` (>= 18.3) and run `trustlock check --enforce`.
...
- `POLICY-REFERENCE.md` — complete `.trustlockrc.json` option table (no TBD entries)
- `ARCHITECTURE.md` — module map, data flows for check and init
- `examples/configs/production.trustlockrc.json` — strict policy example (valid JSON)
- `examples/configs/relaxed.trustlockrc.json` — permissive policy with `_comment` annotations (valid JSON)
- `examples/ci/github-actions.yml` — GitHub Actions workflow with `trustlock check --enforce`
...
- AC: examples/configs/production.trustlockrc.json is valid JSON → `node -e "JSON.parse(...)"`
- AC: examples/configs/relaxed.trustlockrc.json is valid JSON → `node -e "JSON.parse(...)"`
...
- AC: All trustlock command examples run against real implementation → smoke test
...
- AC: `examples/configs/production.trustlockrc.json` is valid JSON → PASS — `node -e "JSON.parse(...)"` returned `valid`
- AC: `examples/configs/relaxed.trustlockrc.json` is valid JSON with annotated permissive settings → PASS — `node -e "JSON.parse(...)"` returned `valid`; `_comment` fields annotate each relaxed setting
- AC: `examples/ci/github-actions.yml` is valid YAML, runs `trustlock check --enforce`, Node >= 18.3 → PASS — `yaml.safe_load()` succeeded; uses `node-version: '18.x'` and `trustlock check --enforce`
...
- AC: All `trustlock` command examples run against real implementation → PASS — `node src/cli/index.js` responds with usage text (exit 2 without command, as expected)
