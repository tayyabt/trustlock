...
The generated approval command printed by `trustlock check` should use the same short rule-name format accepted by `trustlock approve`. For example:
```
trustlock approve scripted-pkg@1.0.0 --override scripts --reason "..." --expires 7d
```
...
1. Initialize a trustlock project: `trustlock init`
2. Add a dependency with install scripts to `package-lock.json`
3. Run `trustlock check` — output includes block reason and generated approval command with `--override 'execution:scripts'`
...
- `trustlock approve` argument parsing (`src/cli/commands/approve.js` or `src/cli/args.js`)
...
Callee: `trustlock approve` parses `--override` values against a known short-name registry.
...
- `trustlock check` output for a blocked `execution:scripts` finding includes `--override scripts` (not `--override 'execution:scripts'`) in the generated approval command
- `trustlock check` output for a blocked `trust:cooldown` finding includes `--override cooldown` in the generated command
...
trustlock check | grep "trustlock approve"
