...
**Default allowlist** (packages verified to legitimately use lifecycle scripts, last updated 2026-04-14):

```
@parcel/watcher, @swc/core, bcrypt, better-sqlite3, bufferutil, canvas,
core-js, cpu-features, deasync, esbuild, electron, fsevents, grpc,
leveldown, node-sass, protobufjs, puppeteer, re2, sharp, sqlite3,
usb, utf-8-validate
```

These packages were verified against the npm registry to confirm active use of `preinstall`, `install`, or `postinstall` scripts. Packages that switched to pre-built binaries (e.g. `@biomejs/biome`, `lightningcss`, `serialport`, `sodium-native`) are intentionally excluded.

**Example (add your own packages to the list):**
```json
{
  "scripts": {
    "allowlist": ["@parcel/watcher", "@swc/core", "my-native-addon"]
  }
}
```

Set to an empty array `[]` to block all packages with install scripts regardless of whether they appear in the default list.
...
This is the exact policy written by `trustlock init` (no flags). See [`examples/trustlockrc-production.json`](examples/trustlockrc-production.json) for a strict production policy and [`examples/trustlockrc-relaxed.json`](examples/trustlockrc-relaxed.json) for a permissive greenfield policy.
